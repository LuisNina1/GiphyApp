...
import * as gifActions from "../actions/get-gifs.actions";
...
describe('useGifs', () => {
  ...
  test('should return a list of gifs when handleTermsClicked', async () => {
    const { result } = renderHook(() => useGifs())
    await act(async () => {
  ...
  test('should return a list of gifs from cache', async () => {
    ...
    vi.spyOn(gifActions, 'getGifsByQuery')
      .mockRejectedValue(new Error('Cusmtom error'))
  })

  test('should retrun on more than 8 previus terms', async () => {
    const { result } = renderHook(() => useGifs())
